...
test('script and storyboard prompts tell the agent the full pipeline', async () => {
  const { scriptPrompt, storyboardPrompt, startPrompt, DEFAULT_STORYBOARD_CONFIG, DEFAULT_SCRIPT_CONFIG, DEFAULT_UI, cutParamsFromUi } = await import('../src/client/prefs.js')
  const script = scriptPrompt('E:/clip.mp4')
  assert.match(script, /bcc-script/)
  assert.match(script, /cutMethod=interval/)
  assert.match(script, /intervalSec=1/)
  assert.match(script, /提取提示词/)
  assert.match(script, /合并提示词/)
  assert.match(script, /remaining=0/)
  const withMaster = scriptPrompt('E:/clip.mp4', DEFAULT_SCRIPT_CONFIG, 'shanzhiyin')
  assert.match(withMaster, /bcc_master/)
  const custom = scriptPrompt('E:/clip.mp4', { ...DEFAULT_SCRIPT_CONFIG, extractPrompt: '只写字幕' })
  assert.match(custom, /只写字幕/)
  ...
  const started = startPrompt({ ...DEFAULT_UI, videoPath: 'E:/clip.mp4', mode: 'storyboard' })
  assert.match(started, /hardMin=5.5/)
  const cut = cutParamsFromUi(DEFAULT_STORYBOARD_CONFIG)
  assert.equal(cut.method, 'smart')
  assert.equal(cut.smart_hard_ratio, 8)
})

test('intervalCutTimes covers the whole video at 1s by default', async () => {
  const { intervalCutTimes } = await import('../src/lib/cuts.js')
  const times = intervalCutTimes(10, 1)
  assert.equal(times[0], 0)
  assert.ok(times.length >= 10)
  assert.ok(times[1] === 1)
})

test('startPrompt requires a video path', async () => {
  const { startPrompt, DEFAULT_UI } = await import('../src/client/prefs.js')
  assert.throws(() => startPrompt({ ...DEFAULT_UI, videoPath: '  ' }), /视频地址/)
})
